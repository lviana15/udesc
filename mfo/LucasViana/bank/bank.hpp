...
struct Investment {
  ...
  bool operator!=(const Investment &other) const { return !(*this == other); }
};
...
struct BankState {
  ...
  bool operator==(const BankState &other) const {
    return (balances == other.balances && investments == other.investments &&
            next_id == other.next_id);
  }
  bool operator!=(const BankState &other) const { return !(*this == other); }
};
...
string sell_investment(BankState &bank_state, string seller,
                       int investment_id) {
  if (bank_state.investments.find(investment_id) ==
      bank_state.investments.end()) {
