...
int pesquisaBinariaB(NoB *no, int chave, int *contador) {
...
int localizaChaveB(ArvoreB *arvore, int chave, int *contador) {
  NoB *no = arvore->raiz;
...
NoB *localizaNoB(ArvoreB *arvore, int chave, int *contador) {
  NoB *no = arvore->raiz;
...
NoB *divideNoB(ArvoreB *arvore, NoB *no, int *contador) {
  int meio = no->total / 2;
...
void adicionaChaveNoB(NoB *no, NoB *novo, int chave, int *contador) {
  int i = pesquisaBinariaB(no, chave, contador);
...
int adicionaChaveRecursivoB(ArvoreB *arvore, NoB *no, NoB *novo, int chave) {
  int contador = 1;
  ...
  if (transbordoB(arvore, no)) {
    ...
    if (no->pai == NULL) {
      ...
      pai->filhos[0] = no;
      adicionaChaveNoB(pai, novo, promovido, &contador);
...
int adicionaChaveB(ArvoreB *arvore, int chave) {
  int contador = 0;
  NoB *no = localizaNoB(arvore, chave, &contador);

  contador += adicionaChaveRecursivoB(arvore, no, NULL, chave);
  return contador;
}
...
void removerChaveB(ArvoreB *arvore, int chave) {
  int contador;
  if (localizaChaveB(arvore, chave, &contador)) {
...
void free_nodes(NoB *node, int ordem) {
    if (node == NULL) {
        return;
    }

    for (int i = 0; i <= node->total; i++) {
        free_nodes(node->filhos[i], ordem);
    }
...
void free_b_tree(ArvoreB *arvore) {
    if (arvore == NULL) {
        return;
    }

    free_nodes(arvore->raiz, arvore->ordem);
